...
function createAndCheckData() {
    ...
    if (_cfg.os == 'win32') {
        _cfg.dataPath = process.env.APPDATA;
    } else if (_cfg.os == 'darwin') {
        _cfg.dataPath = process.env.HOME + '/Library/Preferences';
    } else if (_cfg.os == 'linux') {
        _cfg.dataPath = process.env.HOME + '/.config';
    } else {
        _cfg.dataPath = '/var/local';
    }
    _cfg.dataPath += '/BetterDiscord/';
